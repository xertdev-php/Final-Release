            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form autocomplete="off" method="post" action="{{route("city.update",$row->id)}}">
                            {{csrf_field()}}
                            {{method_field("PATCH")}}
                            <div class="card-body">
                                    ...
                                    <select class="form-control" name="state" id="state">
                                        ...
                                        @if(!$state->isEmpty())
                                            @foreach($state as $state_row)
                                                <option value="{{$state_row->id}}" @if($row->state == $state_row->id) {{"selected"}} @endif>{{$state_row->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
